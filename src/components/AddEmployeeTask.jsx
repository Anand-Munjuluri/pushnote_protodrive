...
export default function AddTask(props) {
  ...
  const addTaskToEmployee = async (employeeEmail, taskDetails) => {
    try {
      const employeeRef = collection(db, 'employees');
      const querySnapshot = await getDocs(query(employeeRef, where('email', '==', employeeEmail)));
  ...
  const handleSubmit = (event) => {
    ...
    addTaskToEmployee(employee.email, taskDetails);
  ...
  return (
    <div className='task-board'>
      ...
      <div className="add-task-form">
        ...
        <button id = "add-task-employee" type="submit" onClick={handleSubmit}>Add Task</button>
      </div>
    </div>
  );
}